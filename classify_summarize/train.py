...
from sklearn.feature_extraction.text import TfidfVectorizerfrom nltk.corpus import stopwords
stop_words = set(stopwords.words('english'))from nltk.stem import WordNetLemmatizer
from sklearn.linear_model import SGDClassifierimport pickle
...
vectorizer = TfidfVectorizer(max_df=0.9, min_df=1, stop_words='english')vectors = vectorizer.fit_transform(df['article'])
# Save the vectorizer to a file
with open(vectorizer_filename, 'wb') as file:    pickle.dump(vectorizer, file)
# Train the SGDClassifier on the whole dataset
sgd_classifier = SGDClassifier()sgd_classifier.fit(vectors, df['category'])
# Save the trained model to a file
with open('sgd_classifier_model.pkl', 'wb') as file:    pickle.dump(sgd_classifier, file)
